...
def embedding_saver(session, step, image_embedding):
  if step == 199:
    saver = tf.train.Saver()
    saver.save(session, os.path.join(FLAGS.summaries_dir, "model.ckpt"), step)
...
def main(_):
  ...
  image_embedding = tf.Variable(tf.random_normal([1370, 10]), name='images')
  ...
  for i in range(FLAGS.how_many_training_steps):
    embedding_saver(sess, i, image_embedding)
    # Let us embed to visualize this
  ...
  embedding.tensor_name = image_embedding.name
  # Link this tensor to its metadata file (e.g. labels).
  embedding.metadata_path = '$LOG_DIR/metadata.tsv'
  embedding.sprite.image_path = '/root/machine7/heroes.jpg'
  # Specify the width and height of a single thumbnail.
  embedding.sprite.single_image_dim.extend([32, 32])


  # Use the same LOG_DIR where you stored your checkpoint.
  summary_writer = tf.summary.FileWriter(FLAGS.summaries_dir)

  # The next line writes a projector_config.pbtxt in the LOG_DIR. TensorBoard will
  # read this file during startup.
  projector.visualize_embeddings(summary_writer, config)

  metadata_file = open(os.path.join(FLAGS.summaries_dir, 'metadata.tsv.tsv'), 'w')
  print(ALL_LABELS)
  for row in xrange(len(ALL_LABELS)):
